# Writing a practice page

A practice page is the claim behind a package, written so someone who has never seen the servers it came from can judge it.
...
Tiers: `observed` (seen in one server), `repeated` (independently arrived at in two or more), `tested` (the claim has a failing test without the practice).

**Adopt.** The two or three calls a server makes to use the package.

**Not for.** The cases where the practice hurts or does not apply.
```
